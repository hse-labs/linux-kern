...
struct keylog_data {
	...
	spinlock_t lock; // переменная блокировки
} k_data;
...
static irqreturn_t my_interrupt( int irq, void *dev_id ) {
	...
	if (pressed=is_key_pressed(scancode)) {
		printk( KERN_INFO "07.4 - In the ISR: counter = %d, scancode: %d, char: %c pressed\n", \
		...
		printk( KERN_INFO "07.4 - In the ISR: counter = %d, scancode: %d, char: %c released\n", \
...
static int __init my_init( void ) {
	...
	if ( request_irq( irq, my_interrupt, IRQF_SHARED, "my_interrupt", &k_data ) )
		return -1;
	printk( KERN_INFO "07.4 - Successfully loading ISR handler on IRQ %d\n", irq );
