...
int __init my_tcp_sock_init(void)
{
	...
	if (err < 0) {
		printk(KERN_WARNING "08-5 - can't create socket\n");
		goto out;
	}

	err = sock->ops->bind(sock, (struct sockaddr *) &addr, addrlen);
	if (err < 0) {
		printk(KERN_WARNING "08-5 - can't bind socket\n");
		goto out_release;
	}

	err = sock->ops->listen(sock, LISTEN_BACKLOG);
	if (err < 0) {
		printk(KERN_WARNING "08-5 - can't listen on socket\n");
		goto out_release;
	}
	printk(KERN_INFO "08-5 - Listening on socket.\n");

	err = sock_create_lite(PF_INET, SOCK_STREAM, IPPROTO_TCP, &new_sock);
	if (err < 0) {
		printk(KERN_WARNING "08-5 - can't create socket for accepted connection\n");
		goto out;
	}
	...
	if (err < 0) {
		printk(KERN_WARNING "08-5 - can't accept new connection\n");
		goto out_release_new_sock;
	}
	err = sock->ops->getname(new_sock, (struct sockaddr *) &raddr, 1);
	if (err < 0) {
		printk(KERN_WARNING "08-5 - can't get peer name\n");
		goto out_release_new_sock;
	}
	printk(KERN_INFO "08-5 - Connection established to %pI4:%d\n", \
			&raddr.sin_addr.s_addr, ntohs(addr.sin_port));

	err = my_tcp_msgsend(new_sock, testmessage);
	if (err < 0) {
		printk(KERN_WARNING "08-5 - can't send message\n");
		goto out_release_new_sock;
	}
	return 0;
